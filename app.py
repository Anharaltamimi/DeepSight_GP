...
load_dotenv()  # يقرأ ملف .env الموجود بجانب app.py ويحمّل المتغيرات في os.environ
...
def get_env(name: str, default: str | None = None, required: bool = False) -> str | None:
    ...
    if required and not value:
        raise RuntimeError(
            f"Missing required environment variable '{name}'. Make sure it is set in your .env file"
        )
    return value
...
# ===============================
GMAIL_ADDRESS = get_env("GMAIL_ADDRESS", required=True)
...
# ===============================
app = Flask(__name__)
app.secret_key = get_env("SECRET_KEY", required=True)
...
# Sessions
# ===============================
SESSION_TYPE = get_env("SESSION_TYPE", default="filesystem")
...
if SESSION_TYPE == "filesystem":
    ...
    import redis
    app.config["SESSION_REDIS"] = redis.from_url(get_env("REDIS_URL", required=True))
...
limiter = Limiter(
    ...
    storage_uri=get_env("RATELIMIT_STORAGE_URL", default="memory://"),
)
...
# Auth & basic pages
# ===============================
TEST_MODE = get_env_bool("TEST_MODE", default=False)
TEST_OTP  = get_env("TEST_OTP", default="654321")


def verify_and_upgrade_password(doctor: "Doctor", plain_password: str) -> bool:
    stored = doctor.Password or ""
...
def login_post():
    ...
    if not doc or not verify_and_upgrade_password(doc, password):
    ...
    if doc.Email:
        ...
        if not sent_ok:
            print("OTP email send failed for doctor:", doc.Doctor_ID)
    else:
        print("No email on file for doctor:", doc.Doctor_ID)
...
def resend_otp():
    ...
    if to_email:
        ...
        if not sent_ok:
            print("OTP resend failed for session doctor:", session.get("doctor_id"))
    else:
        print("No email in session for OTP resend, doctor:", session.get("doctor_id"))
...
if __name__ == "__main__":
    app.run(debug=get_env_bool("FLASK_DEBUG", default=False))